...
the need for the LLM to call a tool — a deliberate design decision that prevents
the small model (llama3.2 3B) from getting stuck in a tool-calling loop.
...
> **Note on response quality:** The response above was generated by llama3.2:3B running
> locally via Ollama. A larger model (7B–13B) produces noticeably richer and more
> philosophically-grounded responses. The persona voice, citations, and sign-off are
> controlled entirely by the agent's `backstory` and `temperature` fields in
> `response_writer.py` — no retraining required.
...
LLM timing is on `llama3.2:latest` (3B) running natively on Apple Silicon via Ollama.
A larger model (7B–13B) would produce higher-quality responses at similar or better speed.
